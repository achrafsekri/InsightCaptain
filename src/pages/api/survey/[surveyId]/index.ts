  ...
  if (req.method === "GET") {
    try {
      ...
      if (!getSurvey) {
        return res
          .status(404)
          .json(formatResponse(null, "Survey not found", "NOT_FOUND"));
      }
    ...
    if (req.method === "PATCH") {
      try {
        ...
        return res
          .status(200)
          .json(formatResponse(updateSurvey, "Success", "OK"));
      } catch {
        res
          .status(500)
          .json({ message: "Something went wrong : Could'nt update survey" });
      }
    }
    ...
    if (req.method === "DELETE") {
      try {
        ...
        return res
          .status(200)
          .json(formatResponse(deleteSurvey, "Success", "OK"));
      } catch {
        res
          .status(500)
          .json({ message: "Something went wrong : Could'nt delete survey" });
      }
    }
  }
}
